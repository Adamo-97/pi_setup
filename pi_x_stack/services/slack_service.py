...
Slack Service
=============
Sends X/Twitter video previews and scripts to Slack for approval.
...
logger = logging.getLogger("x.slack")


class SlackService:
    """Slack notifications with Block Kit for X/Twitter pipeline."""

    def __init__(self, webhook_url: str, n8n_base_url: str = "http://localhost:5681"):
    ...
    def send_approval_request(
        ...
        """
        Send a rich Slack message for X video approval.
        ...
        blocks: List[Dict[str, Any]] = [
            {
                ...
                "text": {
                    "type": "plain_text",
                    "text": f"🐦 X Video Ready — {content_type.replace('_', ' ').title()}",
                },
            },
            {"type": "divider"},
            {
    ...
    def send_status(self, message: str, level: str = "info") -> bool:
        ...
        payload = {"text": f"{emoji} *X/Twitter Pipeline:* {message}"}
        return self._send(payload)
    ...
    def send_publish_confirmation(
        ...
        blocks = [
            {
                "type": "header",
                "text": {"type": "plain_text", "text": "🚀 Published to X!"},
            },
            {
                "type": "section",
                "fields": [
    ...
    def send_error(self, step: str, error: str) -> bool:
        ...
        blocks = [
            {
                "type": "header",
                "text": {"type": "plain_text", "text": "🔴 X Pipeline Error"},
            },
            {
                "type": "section",
                "fields": [
