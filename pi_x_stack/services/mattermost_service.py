...
Mattermost Service
====================
Sends X/Twitter video previews and scripts to Mattermost for approval.
...
logger = logging.getLogger("x.mattermost")


class MattermostService:
    """Mattermost REST API client for X/Twitter pipeline notifications."""
    ...
    def __init__(
        ...
        bot_token: str,
        channel_id: str,
        n8n_base_url: str = "http://localhost:5681",
    ):
    ...
    def send_gate_approval(
        ...
        approve_url = (
            f"{self.n8n_base_url}/webhook/x-approve"
            f"?gate={gate_number}&run_id={run_id}&action=approve"
        )
        reject_url = (
            f"{self.n8n_base_url}/webhook/x-reject"
    ...
    def send_approval_request(
        ...
        """
        Send a rich Mattermost message for X video approval.
        ...
        """
        approve_url = f"{self.n8n_base_url}/webhook/x-approve"
        reject_url = f"{self.n8n_base_url}/webhook/x-reject"
        ...
        message = (
            f"### :bird: X Video Ready — {content_type.replace('_', ' ').title()}\n\n"
            f"| Field | Value |\n"
            f"|:------|:------|\n"
    ...
    def send_status(self, message: str, level: str = "info") -> bool:
        ...
        return self._post_message(f"{emoji} **X/Twitter Pipeline:** {message}")
    ...
    def send_publish_confirmation(
        ...
        """Send confirmation that video was published to X via Buffer."""
        message = (
            f"### :rocket: Published to X!\n\n"
            f"| Field | Value |\n"
            f"|:------|:------|\n"
            f"| **Title** | {title} |\n"
