...
Usage:
    python -m pipeline.step7_publish_x --video-id <UUID> [--mode notify|publish]
"""
...
def main(video_id: str, mode: str = "notify") -> dict:
    """
    Publish X/Twitter video or send for approval.
    ...
    rows = execute_query(
        ...
        SELECT rv.id, rv.script_id, rv.file_path, rv.duration, rv.status,
               gs.script_text, gs.content_type, gs.tweet_text
...
def _publish_to_buffer(
    ...
    """Publish video to X/Twitter via Buffer."""
    buffer = BufferService(
        access_token=settings.buffer.access_token,
        profile_id=settings.buffer.profile_id,
    )

    # Use tweet caption if available, otherwise generate from script
    if tweet_text:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Publish X/Twitter video")
