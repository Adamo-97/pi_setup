...
def main(content_type: str = "trending_news", duration: float = 45.0) -> dict:
    """
    Generate an X/Twitter video script.
    ...
    Returns:
        dict with script_id, script_text, tweet_text, word_count, etc.
    """
    ...
    # Save tweet text if present
    if result.get("tweet_text"):
        tweet_file = output_dir / f"{result['script_id'][:8]}_tweet.txt"
        tweet_file.write_text(result["tweet_text"], encoding="utf-8")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate X/Twitter video script")
