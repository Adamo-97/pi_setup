...
DIRS=(
    ...
    "$NVME_MOUNT/mattermost/push-proxy/config"
    "$NVME_MOUNT/mattermost/push-proxy/certs"
)
...
# ── Step 4b: Push Proxy Config ───────────────────────────────────────────
PROXY_CONFIG_DIR="$NVME_MOUNT/mattermost/push-proxy/config"
PROXY_CONFIG="$PROXY_CONFIG_DIR/mattermost-push-proxy.json"
TEMPLATE="$PROJECT_DIR/config/push-proxy.json.example"

if [[ -f "$PROXY_CONFIG" ]]; then
    warn "Push proxy config already exists — skipping (edit $PROXY_CONFIG manually)"
else
    if [[ -f "$TEMPLATE" ]]; then
        sudo cp "$TEMPLATE" "$PROXY_CONFIG"
        sudo chmod 644 "$PROXY_CONFIG"
        log "Push proxy config copied to $PROXY_CONFIG"
    else
        warn "Template not found: $TEMPLATE"
    fi
    warn "IMPORTANT: Edit push proxy config before mobile push will work:"
    warn "  sudo nano $PROXY_CONFIG"
    warn "  Android: Replace REPLACE_WITH_YOUR_FCM_SERVER_KEY with your Firebase FCM Server Key"
    warn "  iOS:     Place your Apple .pem at: $NVME_MOUNT/mattermost/push-proxy/certs/apple.pem"
fi
...
echo "  │  Tailscale: http://$(tailscale ip -4 2>/dev/null || echo 'N/A'):${MATTERMOST_PORT:-8065}      │"
echo "  │  API:       http://${DETECTED_IP:-$HOST_IP}:${MATTERMOST_PORT:-8065}/api/v4  │"
echo "  │  PgSQL:     localhost:${POSTGRES_PORT:-5438}                     │"
echo "  │  PushProxy: http://${DETECTED_IP:-$HOST_IP}:${PUSH_PROXY_PORT:-8066}         │"
echo "  └─────────────────────────────────────────────────────┘"
echo ""
info "NEXT STEPS:"
echo "  1. Open http://$(tailscale ip -4 2>/dev/null || echo "$HOST_IP"):${MATTERMOST_PORT:-8065} in browser"
...
echo "     Then: Admin Console → Notifications → Mobile Push → verify server URL"
echo ""
info "TAILSCALE ACCESS:"
echo "  Tailscale IP: $(tailscale ip -4 2>/dev/null || echo 'Run: tailscale ip -4')"
echo "  MM_SITEURL is already set in .env — confirm with: grep MM_SITEURL .env"
echo "  If the Tailscale IP changes: edit .env → restart: docker compose restart mattermost"
echo ""
info "PUSH PROXY SETUP (self-hosted):"
echo "  Config: $NVME_MOUNT/mattermost/push-proxy/config/mattermost-push-proxy.json"
echo ""
echo "  Android (Google FCM):"
echo "    1. https://console.firebase.google.com → Project Settings → Cloud Messaging"
echo "    2. Copy Server Key → replace REPLACE_WITH_YOUR_FCM_SERVER_KEY in config above"
echo ""
echo "  iOS (Apple APNs):"
echo "    1. Apple Developer account required"
echo "    2. Create APNs cert for com.mattermost.react.native → export as .pem"
echo "    3. sudo cp your-cert.pem $NVME_MOUNT/mattermost/push-proxy/certs/apple.pem"
echo ""
echo "  After setting credentials:"
echo "    docker compose restart push-proxy"
echo ""
