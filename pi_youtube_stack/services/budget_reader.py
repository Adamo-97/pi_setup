...
class BudgetReader:
    ...
    def __init__(
        ...
        self._nextcloud_user = nextcloud_user or os.getenv("NEXTCLOUD_USER", "admin")
        self._nextcloud_password = nextcloud_password or os.getenv(
            "NEXTCLOUD_PASSWORD", ""
        )
    ...
    def _load_from_nextcloud(self) -> Optional[Dict[str, Any]]:
        ...
        url = f"{self._nextcloud_url}/{self._nextcloud_user}/" f"{self._nextcloud_path}"
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                logger.debug("Nextcloud returned %d for budgets.json", resp.status_code)
        except Exception as exc:
