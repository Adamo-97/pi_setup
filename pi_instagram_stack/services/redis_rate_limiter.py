...
class RedisRateLimiter:
    ...
    def __init__(
        ...
        try:
            ...
            logger.error("Redis connection failed: %s — Bouncer running in PERMISSIVE mode", exc)
            self._redis = None
    ...
    def set_budget_limit(self, weekly_units: int) -> None:
        ...
        logger.info(
            "Budget limit set: %s = %d units/week", self.platform, weekly_units
        )
    ...
    def check_and_consume(self, api_name: str, units: Optional[int] = None) -> bool:
        ...
        try:
            ...
            if self._budget_limit is not None:
                ...
                if (current_total + units) > self._budget_limit:
                    logger.warning(
                        ...
                        self.platform, api_name, units,
                        current_total, self._budget_limit,
                    )
                    return False
            ...
            logger.info(
                ...
                self.platform, api_name, units,
                new_total, self._budget_limit or "unlimited",
            )
            return True

        except Exception as exc:
            logger.error(
                "Redis error during budget check — PERMISSIVE: %s", exc
            )
            return True
