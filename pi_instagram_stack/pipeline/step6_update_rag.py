...
Usage:
    python -m pipeline.step6_update_rag --script-id <UUID> [--feedback <text>]
"""
...
def main(
    ...
    """
    logger.info("=== Step 6: Update RAG (%s) ===", script_id[:8])

    rag = RAGManager()

    # Fetch script and validation data
    rows = execute_query(
        """
        SELECT gs.id, gs.script_text, gs.content_type,
    ...
    logger.info("RAG updated: %s (%d updates)", script_id[:8], len(updates_done))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Update RAG with pipeline data")
    parser.add_argument("--script-id", required=True, help="Script UUID")
    parser.add_argument("--feedback", default="", help="Optional user feedback")
    parser.add_argument(
        "--feedback-type",
