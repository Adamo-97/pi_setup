...
Buffer Service
==============
Publishes TikTok videos via Buffer API.
...
logger = logging.getLogger("tiktok.buffer")


class BufferService:
    """Buffer API client for TikTok auto-publishing."""
    ...
    def publish_video(
        ...
        """
        Publish a video to TikTok via Buffer.

        Args:
            video_path: Path to the rendered .mp4 file.
            caption: Arabic caption text for TikTok post.
            hashtags: Space-separated hashtag string (e.g. "#gaming #أخبار").
        ...
        try:
            ...
            if resp.status_code == 200 and resp_data.get("success"):
                ...
                return {
                    "success": True,
                    "update_id": update_id,
                    "message": "Video queued for TikTok publishing",
                    "response": resp_data,
                }
            else:
    ...
    def get_default_hashtags(content_type: str) -> str:
        """Get default Arabic gaming hashtags by content type."""
        base = "#قيمنق #ألعاب #gaming #gamer"
        type_tags = {
